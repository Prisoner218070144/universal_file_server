...
def browse_folder(folder_path):
    ...
    return template_renderer.render_browse_page(
        items, safe_path, breadcrumbs, parent_path, 
        file_counts, files_by_type, preview_nav_data
    )
...
def serve_file(file_path):
    ...
    if file_ext in ['.txt', '.py', '.js', '.html', '.css', '.json', '.xml', '.csv', '.md', '.log']:
...
def upload_files(folder_path=''):
    ...
    try:
        ...
        if 'files[]' not in request.files:
            return jsonify({'error': 'No files selected', 'success': [], 'errors': []}), 400
...
def get_preview_data(file_path):
    ...
    if items:
        ...
        if current_index >= 0:
            prev_file = same_type_files[current_index - 1] if current_index > 0 else None
            next_file = same_type_files[current_index + 1] if current_index < len(same_type_files) - 1 else None
...
def get_file_content(file_path):
    ...
    text_extensions = ['.txt', '.py', '.js', '.html', '.css', '.json', '.xml', '.csv', '.md', '.log', '.ini', '.cfg', '.conf']
...
def get_word_document_content(file_path):
    ...
    try:
        file_size = os.path.getsize(full_path)
        if file_size > Config.PERFORMANCE_CONFIG.get('MAX_WORD_PREVIEW_SIZE', 10 * 1024 * 1024):
            return jsonify({
                'error': 'File too large for preview', 
                'size': file_size,
                'max_size': Config.PERFORMANCE_CONFIG.get('MAX_WORD_PREVIEW_SIZE', 10 * 1024 * 1024)
            })
    except:
        pass
