...
class FileSystemModel:
    def __init__(self, root_drive=Config.ROOT_DRIVE):
        ...
        self._cache_timeout = getattr(Config, 'PERFORMANCE_CONFIG', {}).get('CACHE_TIMEOUT', 10)
        self._search_cache = {}
        self._folder_size_cache = {}
        self._executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="FileSystemModel")
        mimetypes.init()
    ...
    def get_folder_contents(self, folder_path):
        ...
        try:
                    ...
                    for entry in entries:
                        ...
                        try:
                            if entry.is_dir():
                                ...
                                items.append(self._create_folder_item_fast(
                                    item_name, 
                                    folder_path
                                ))
                            else:
                                ...
                                try:
                                    ...
                                    items.append(self._create_file_item_fast(
                                        item_name, 
                                        folder_path, 
                                        file_size
                                    ))
            ...
            x.get('name', '').lower() if x.get('name') else ''  # Safe string comparison
        ))
                ...
                with os.scandir(current_path) as entries:
                    for entry in entries:
                        ...
                        try:
                            ...
                            if entry_name.startswith('.') or entry_name in ['$RECYCLE.BIN', 'System Volume Information', 'Thumbs.db']:
                                continue
                            ...
                            if matches:
                                ...
                                try:
                                    rel_path = os.path.relpath(entry.path, self.root_drive)
                                except ValueError:
                                ...
                                if entry.is_dir():
                                    ...
                                    results.append(self._create_folder_item(
                                        entry_name, 
                                    ...
                                    try:
                                        ...
                                        results.append(self._create_file_item(
                                            entry_name, 
                                            entry.path, 
                                            os.path.dirname(rel_path),
                                            file_size
                            ...
                            if entry.is_dir():
                                ...
                                if entry_name.lower() not in ['windows', 'program files', 'program files (x86)', 'system32']:
    ...
    def _clean_search_cache(self):
        ...
        for key, (_, timestamp) in self._search_cache.items():
            if current_time - timestamp > self._cache_timeout * 5:  # 5x longer cache for searches
    ...
    def count_file_types(self, items):
        ...
        counts = {
            'folders': 0, 'videos': 0, 'audios': 0, 
            'images': 0, 'documents': 0, 'text': 0, 'others': 0
        }
